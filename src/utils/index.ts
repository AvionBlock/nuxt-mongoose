...
export function useViteWebSocket(nuxt: Nuxt = useNuxt()) {
  return new Promise<
    ReturnType<typeof nuxt.hooks.hook> extends any ? any : never
  >((resolve) => {
...
export const ${name}SchemaDef = ${outputObject}

export const ${name}Schema = defineMongooseModel({
  name: '${name}',
  schema: ${name}SchemaDef,
})
`;
}
...
export function generateApiRoute(
  action: string,
  { model, by }: { model: { name: string; path: string }; by?: string },
) {
