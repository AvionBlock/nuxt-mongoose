...
export default defineNuxtModule<ModuleOptions>({
  ...
  async setup(_options, _nuxt) {
    ...
    if (_nuxt.options.dev) {
      $fetch("https://registry.npmjs.org/nuxt-mongoose/latest")
        .then((release) => {
    ...
    _nuxt.hook("prepare:types", (options) => {
      options.references.push({
        path: resolve(_nuxt.options.buildDir, "types/nuxt-mongoose.d.ts"),
      });
    });

    const isDevToolsEnabled =
      typeof _nuxt.options.devtools === "boolean"
        ? _nuxt.options.devtools
        : _nuxt.options.devtools.enabled;
